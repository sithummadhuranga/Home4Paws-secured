// ---- FIXED (V05): backdoor developer endpoints disabled ----
// POST /api/dev/make-admin, /api/dev/reset-password and /api/dev/create-test-user let ANY
// anonymous caller promote an account to Admin, overwrite any user's password, or create
// users and receive their tokens. The only guard was IsDevelopment(), and the app runs in
// Development by default (launchSettings.json, docker-compose.yml), so they were live
// backdoors (OWASP A05/A07, CWE-489). GET /api/dev/health also exposed the environment name.
// The whole controller is kept below as comments for traceability and is no longer compiled
// or routed; every /api/dev/* request now returns 404.
// The user list moved to the Admin-only GET /api/admin/users (see V02, AdminUsersController).
// ---- CODE BEFORE FIX (V05) ----
